...
from antinode_norma.auth.middleware import ensure_resource_owner
...
def _client_with_user(user: User | None) -> TestClient:
    """Helper to return a TestClient with request.state.user pre-populated."""
    test_app = FastAPI()
    ...
    # Mount v1 and legacy routes from main app
...
def test_viewer_role_route_matrix():
    ...
    client = _client_with_user(viewer)
...
def test_generator_role_route_matrix():
    ...
    client = _client_with_user(generator)
...
def test_reviewer_role_route_matrix():
    ...
    client = _client_with_user(reviewer)
