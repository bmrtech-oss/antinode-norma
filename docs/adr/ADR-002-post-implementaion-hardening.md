...
4. **Score credibility.** Replace the 9.53 point estimate with the corrected three-column model (projected 9.03 / delivered 8.61 / target 9.09).
5. **Security posture correction.** Remove `read:secrets` from the plugin model; introduce brokered secret interface and subprocess isolation.
6. **Enterprise readiness.** Deliver DR, observability, load testing, API versioning, i18n externalization, v4→v5 migration.
7. **Timeline realism.** Publish P50/P80 dates. Add a 1-week convergence buffer.
...
**Exit criteria:** All six deliverables merged; DR drill performed once; load test baseline published.
...
- **Brokered secret interface**: plugins request a secret by name; the broker injects at call time, logs access, enforces a per-plugin allowlist.
- **Subprocess isolation**: each plugin runs in a subprocess with restricted filesystem (own directory only) and network (declared domains only).
- **Manifest extension**: `plugin.yaml` must declare `allowed_domains`.
...
- Update `docs/PLUGINS.md` with trust model and disclosure policy.
...
**Exit criteria:** Plugin model documented and tested; cache FPR gate active; migration policy updated; MCP tool descriptions updated.
...
**Exit criteria:** CI gate active; release checklist used once; score re-published.
...
**Exit criteria:** v2.1.0 tagged; migration guide published; release notes reviewed.
...
|---|---|---|---|---|
| Gherkin generation quality | 20% | 9.5 | 9.3 | 9.5 |
| Determinism / reproducibility | 12% | 9.5 | 8.7 | 9.5 |
| Traceability | 10% | 9.5 | 9.4 | 9.5 |
| Input flexibility | 8% | 9.0 | 9.0 | 9.0 |
| Enterprise readiness | 10% | 9.5 | 7.8 | 9.5 |
| Codegen | 4% | 8.0 | 8.0 | 8.0 |
| Execution | 8% | 8.5 | 8.3 | 8.5 |
| UI / UX | 10% | 8.5 | 8.2 | 8.5 |
| Collaboration | 5% | 7.5 | 7.4 | 7.8 |
| Ecosystem / plugins | 5% | 8.0 | 7.2 | 8.8 |
| Extensibility | 4% | 9.5 | 9.5 | 9.5 |
| Documentation | 2% | 9.0 | 8.8 | 9.5 |
| Cost efficiency | 2% | 9.0 | 9.0 | 9.0 |
| **Weighted total** | **100%** | **9.03** | **8.61** | **9.09** |
